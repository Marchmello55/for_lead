...
@router.callback_query(F.data.startswith("project_add-state_deadline"))
async def calendar_button(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if "year" in callback.data:
        ...
        if "next_year" in callback.data:
            ...
            await callback.message.edit_reply_markup(reply_markup=await ic.generate_calendar_kb(prefix=prefix, year=year + 1))
        elif "back_year" in callback.data:
            prefix = callback.data.split("_back_year")[0]
            await callback.message.edit_reply_markup(reply_markup=await ic.generate_calendar_kb(prefix=prefix, year=year - 1))
    elif "month" in callback.data:
        month_year = str(callback.data.split("month_")[1])
        ...
        if "next_month" in callback.data:
            ...
            await callback.message.edit_reply_markup(reply_markup=await ic.generate_calendar_kb(prefix=prefix, year=year + 1, month=month + 1))
        elif "back_month" in callback.data:
            prefix = callback.data.split("_back_month")[0]
            await callback.message.edit_reply_markup(reply_markup=await ic.generate_calendar_kb(prefix=prefix, year=year, month=month - 1))
    else:
